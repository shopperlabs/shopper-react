...
jQuery.fn.slugIt = function(options) {
  ...
  let opts  = jQuery.extend(defaults, options);
  ...
  jQuery(this).bind(defaults.events, function() {
    ...
    let sep_esc  = opts.separator.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1");
    let re_trail = new RegExp('^'+ sep_esc +'+|'+ sep_esc +'+$', 'g');
    let re_multi = new RegExp(sep_esc +'+', 'g');
