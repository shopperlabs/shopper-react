...
export default class CategoryForm extends ShopperComponent {
  constructor(props) {
    super(props)
  ...
  componentDidMount() {
    ...
    if (window.location.href !== route('shopper.catalogue.categories.create').template) {
      this.getRecord()
    }
  }
  ...
  getCategories() {
    if (window.location.href === route('shopper.catalogue.categories.create').template) {
      ...
      let element = document.getElementById('category-form')

      axios
        .get(route('shopper.catalogue.categories.list', {id: parseInt(element.getAttribute('data-id'))}))
        .then((response) => {
          this.setState({categories: response.data})
        })
        .catch((error) => {
  ...
  getRecord() {
    ...
    axios
      .get(route('shopper.catalogue.categories.show', {id: parseInt(element.getAttribute('data-id'))}))
      .then((response) => {
  ...
  updateForm(id) {
    axios
      .put(route('shopper.catalogue.categories.update', {id: parseInt(id)}), this.state.form)
      .then((response) => {
        ...
        setInterval(function() {
          window.location = route('shopper.catalogue.categories.index')
        }, 1500)
      })
      .catch((error) => {
        this.setState({loader: false})
  ...
  postForm() {
    axios
      .post(route('shopper.catalogue.categories.store'), this.state.form)
      .then((response) => {
        this.setState({loader: false})
        Notification({
          title: response.data.title,
    ...
    return (
      ...
      >
          ...
          <Loading text={this.trans.get('Loading Data...')} loading={this.state.loading}>
            ...
            <Tabs activeName='1' type='card'>
              <Tabs.Pane label={this.trans.get('Settings')} name='1'>
                <Layout.Row gutter="20">
                  <Layout.Col span="12">
                    <Form.Item label={this.trans.get('Parent')}>
                      <Select
        ...
        <div className="wrapper-md buttons-actions">
          ...
          <a href={route('shopper.catalogue.categories.index')}>{this.trans.get('Cancel')}</a>
        </div>
      </Form>
    )
  }
...
if (document.getElementById('category-form')) {
  ReactDOM.render(<CategoryForm />, document.getElementById('category-form'))
}
